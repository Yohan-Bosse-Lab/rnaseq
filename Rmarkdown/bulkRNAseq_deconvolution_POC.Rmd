...
params:
  ...
  deconvolution_POC: 'C:/Users/renseb01/Documents/rnaseq/deconvolution_POC'
  cibersortX: FALSE
---
...
if (!dir.exists(params$deconvolution_POC)){ 
  warning(paste0('Dir: ',params$deconvolution_POC,' does not exist. Please check')) }

if (!dir.exists(file.path(params$deconvolution_POC,'music'))){ 
  warning(paste0(file.path(params$deconvolution_POC,'music'),' does not exist. I will create it')) 
  dir.create(file.path(params$deconvolution_POC,'music'))
  }

if (!dir.exists(file.path(params$deconvolution_POC,'cibersortX'))){ 
  warning(paste0(file.path(params$deconvolution_POC,'cibersortX'),' does not exist. I will create it')) 
  dir.create(file.path(params$deconvolution_POC,'cibersortX'))
  }
```
...
for(i in 1:6){
  ...
  write.csv(cell_type_annotations_Xpercent,file.path(params$deconvolution_POC,paste0('music/cell_type_annotations_',levels[i],'_Xpercent.csv')))
  write.csv(variable_singlecelldata_Xpercent,file.path(params$deconvolution_POC,paste0('music/variable_singlecelldata_',levels[i],'_Xpercent.csv')))
  ...
  if(params$cibersortX) {
    ...
    write.table(variable_singlecelldata_Xpercent,file.path(params$deconvolution_POC,paste0('cibersortX/variable_singlecelldata_',levels[i],'_Xpercent_cibersortx.txt')),sep = '\t',quote = F,row.names = F)
    ...
    if(i == 1) {
      ...
      write.table(bulk_cibersort,file.path(params$deconvolution_POC,paste0('cibersortX/bulk_cibersortx.txt')),sep = '\t',quote = F,row.names = F)
    }
  }
...
```{r real data deconvolution}
  ...
  if(!file.exists(file.path(params$datapath,'results/deconvolution/deconv_music.RDS'))){
  
  for(i in 1:6){
  #2.Reference data
  cell_type_annotations_Xpercent = read.csv(file.path(params$deconvolution_POC,paste0('music/cell_type_annotations_',levels[i],'_Xpercent.csv')), row.names = 1)
  variable_singlecelldata_Xpercent = as.matrix(read.csv(file.path(params$deconvolution_POC,paste0('music/variable_singlecelldata_',levels[i],'_Xpercent.csv')),row.names = 1))
  ...
  #bulk = bulk[rmean>quantile(rmean,seq(0,1,by=0.1))[10],]
 ...
 # }
  ...
  #set_cibersortx_credentials('sebastien.renaut.1@ulaval.ca','d184d0ae37a164c35f463438cf6e1daa')
...
#{
  ...
  saveRDS(deconv_music,file.path(params$datapath,'results/deconvolution/deconv_music.RDS'))

  } else {
    deconv_music = readRDS(file.path(params$datapath,'results/deconvolution/deconv_music.RDS'))
}
```
...
# sandbox info 
```{r sandbox,eval = F, message= T}

#
data = pca_tissue$data

#xy = data.frame(x=c(sample(11:20),sample(1:10)), y = c(sample(11:20),sample(1:10)),z = c(rep('A',10),rep('B',10)),s = rep(1:10,2))
#plot(xy[,1:2],col = c(rep('red',10),rep('blue',10)))

centroids = rbind(c(mean(data$x[data$Col.=='Sain']),mean(data$y[data$Col.=='Sain'])),
                 c(mean(data$x[data$Col.=='Tumeur']),mean(data$y[data$Col.=='Tumeur'])))
                 
points(t(centroids[1,]),col = 'darkred',pch = 19)
points(t(centroids[2,]),col = 'darkblue',pch = 19)

xyd = as.matrix(dist(data[,2:3]))
